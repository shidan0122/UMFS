...
%Computing distance between training instances
    dist_matrix=diag(realmax*ones(1,num_training));               %生成一个距离矩阵，对角线上为同一个样本间的距离
...
%Computing Prior and PriorN
    for i=1:num_class
        temp_Ci=sum(train_target(i,:)==ones(1,num_training));           %计算样本中拥有i标记的样本总数
...
%Computing Cond and CondN
    ...
    for i=1:num_training
        [temp,index]=sort(dist_matrix(i,:));%temp存储第i个样本与其他样本的距离（从小到大排序）index存储这些排序后的样本索引
        Neighbors{i,1}=index(1:Num);
    end                             %Neighbors{i,1} 中存放每个样本的邻域索引
    ...
    for i=1:num_training
        ...
        neighbor_labels=[];
        for j=1:Num
            neighbor_labels=[neighbor_labels,train_target(:,Neighbors{i,1}(j))];%记录第i个样本中各个邻域的标记
        end
        for j=1:num_class
            temp(1,j)=sum(neighbor_labels(j,:)==ones(1,Num));                 %得到样本i的邻域中拥有标记j的个数
        end
        for j=1:num_class
